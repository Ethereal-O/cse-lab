...
//
chfs_client::status
fuseserver_createhelper(const char *link, fuse_ino_t parent, const char *name,
                        mode_t mode, struct fuse_entry_param *e, int type)
{
    ...
    if (type == extent_protocol::T_FILE)
        ...
        ret = chfs->symlink(link, parent, name, inum);
    if (ret != chfs_client::OK)
        return ret;
    e->ino = inum;
                       ...
                       mode_t mode, struct fuse_file_info *fi)
{
    ...
    chfs_client::status ret;
    if ((ret = fuseserver_createhelper(nullptr, parent, name, mode, &e, extent_protocol::T_FILE)) == chfs_client::OK)
    {
                        ...
                        const char *name)
{
    struct fuse_entry_param e;
    chfs_client::status ret;
    if ((ret = fuseserver_createhelper(link, parent, name, 0, &e, extent_protocol::T_SYMLINK)) == chfs_client::OK)
    {
        fuse_reply_entry(req, &e);
        printf("OK: symlink returns.\n");
    }
    else
    {
        if (ret == chfs_client::EXIST)
